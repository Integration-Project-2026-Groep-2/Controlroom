...
func TestConsumeCompanies_StopsOnContextCancel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	msgs := make(chan amqp.Delivery)
	done := make(chan struct{})

	go func() {
		company.ConsumeCompanies(nil, msgs, ctx)
		close(done)
	}()
	...
	case <-time.After(time.Second):
		t.Fatal("ConsumeCompanies stopte niet na context cancel")
	}
}

func TestConsumeCompanies_StopsOnClosedChannel(t *testing.T) {
	...
	case <-done:
		// geslaagd
	case <-time.After(time.Second):
		t.Fatal("ConsumeCompanies stopte niet na gesloten channel")
	}
}
