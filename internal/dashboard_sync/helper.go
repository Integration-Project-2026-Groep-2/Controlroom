...
func getActiveServicesLastWeek(es *elasticsearch.Client) ([]string, error) {
	...
	if hits, ok := result["hits"].(map[string]any); ok {
		if total, ok := hits["total"].(map[string]any); ok {
			logger.Log(logger.NewMessage(logger.DEBUG, logger.CONTROLROOM, fmt.Sprintf("total hits: %v", total)))
		} else if totalVal, ok := hits["total"].(float64); ok {
			logger.Log(logger.NewMessage(logger.DEBUG, logger.CONTROLROOM, fmt.Sprintf("I don't know what to put here..., %v", totalVal)))
		}
	}
...
func getAllPanelTitles() (map[string]string, error) {
	reqURL := fmt.Sprintf("%s/api/saved_objects/_find?type=lens&type=visualization&per_page=1000", config.KibanaURL)
	req, err := http.NewRequest("GET", reqURL, nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("kbn-xsrf", config.KibanaUserName)
...
func findSavedObjectByTitle(title string, objectType string) (string, error) {
	...
	reqURL := fmt.Sprintf("%s/api/saved_objects/_find?type=%s&search_fields=title&search=%s", config.KibanaURL, objectType, q)
...
func createOrUpdateSavedObject(objectType string, id string, payload map[string]any) (string, error) {
	method := "POST"
	reqURL := fmt.Sprintf("%s/api/saved_objects/%s", config.KibanaURL, objectType)
...
func syncHeartbeatDashboard(es *elasticsearch.Client) {
	...
	if err != nil {
		logger.Log(logger.NewMessage(logger.DEBUG, logger.CONTROLROOM, fmt.Sprintf("error in getting the services from last week: %s", err)))
		return
	}

	attrs, panels, refs, err := getDashboardAttributes()
	if err != nil {
		logger.Log(logger.NewMessage(logger.DEBUG, logger.CONTROLROOM, fmt.Sprintf("error in declaring the exchanges: %s", err)))
		return
	}
...
func getDashboardAttributes() (map[string]any, []map[string]any, []map[string]any, error) {
	...
	req.Header.Set("kbn-xsrf", config.KbnXsrfToken)
	if u := config.KibanaUserName; u != "" {
		req.SetBasicAuth(u, config.KibanaPassword)
	}
...
func putDashboardAttributes(attributes map[string]any, references []map[string]any) error {
	...
	if u := config.KibanaUserName; u != "" {
		putReq.SetBasicAuth(u, config.KibanaPassword)
	}
...
func InitDashboardSync(client *elasticsearch.Client) {
	if config.KibanaUserName == "" {
		logger.Log(logger.NewMessage(logger.INFO, logger.CONTROLROOM, "Warning: KIBANA_USERNAME not set. Kibana auth will be disabled."))
	}

	ticker := time.NewTicker(5 * time.Second)
	for range ticker.C {
		syncDashboard(client)
		syncHeartbeatDashboard(client)
	}
}
